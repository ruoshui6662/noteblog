...
文档状态：P0–P4 首轮已实现，P5 待实施
日期：2026-09-06  
视觉基准：已选择的图二方案（左侧文件树 + 中央编辑器 + 右侧预览）  
参考产品：Obsidian、思源笔记、Typora

本地验收与问题分析见：[ADMIN_WORKSPACE_TEST_REPORT.md](ADMIN_WORKSPACE_TEST_REPORT.md)。

本轮执行记录：管理端已切换到图二的三栏工作区，完成文件树、上下文新建菜单、文档标签、中央 Markdown 编辑器、实时安全预览、属性面板、未保存状态和旧后端目录接口 404 的路径降级。P3 已加入文档移动、复制、目录移动、空目录删除及树和标签联动；P4 首轮已将文档标题、摘要、标签、排序和草稿状态改为结构化 API，由后端统一写回 Front Matter 并保留未知字段。文档重命名、并发冲突恢复和完整发布工作流仍按后续阶段推进。
...
目标是让“路径变化”在真实数据目录中可验证。先增加独立的文档路径重命名和目录重命名入口，再增加树刷新接口和统一的结构操作响应，响应中返回变更前路径、变更后路径、受影响文档数量和树版本。随后补齐附件目录随文档移动的策略，并用临时数据目录覆盖移动、复制、冲突、非空目录删除和容器重启场景。

验收条件：飞牛绑定的 `/data/content` 中能直接看到预期文件变化；刷新管理端、公共首页和文档链接后路径一致；目标冲突和版本冲突不会丢失源文件。

### 第 2 步：收敛管理 API 与属性模型

把 Vue 中散落的 `fetch` 调用收敛到 `adminApi`，统一路径编码、会话失效、错误码和 ETag。后端负责 Front Matter 的解析与写回，前端属性面板只提交标题、描述、标签、目录、排序和草稿状态等结构化字段，避免字符串拼接破坏 YAML。

验收条件：修改显示标题不改变文件路径；修改属性后预览、搜索摘要和公共阅读端使用同一份保存结果；401、409、422、500 都有明确下一步操作。

### 第 3 步：完善发布与编辑体验

增加草稿/公开状态、保存草稿与发布动作，补齐标题层级大纲、链接和代码块预览。编辑器仍以 Markdown 源码为事实源，只有在快捷键、焦点、冲突恢复和大文档性能稳定后，才评估 CodeMirror 6 和自动保存。

验收条件：草稿不会出现在公共树、搜索和未授权阅读端；发布后无需重启容器即可访问；冲突时保留本地内容并提供服务器版本对照。

### 第 4 步：可靠性、响应式与发布门禁

补充 API 集成测试、浏览器端到端测试和移动端测试；处理外部文件变化、断网恢复、刷新前未保存提示、键盘导航、Escape 关闭和 200% 缩放。每个阶段都执行 `vue-tsc`、Vite 构建、Go 测试、Docker smoke test 和 Compose 校验，通过后再推送 `edge` 镜像。

验收条件：CI 通过后才在飞牛执行 `docker compose pull`；飞牛验证健康检查、真实绑定目录写入、升级后数据保留和回滚路径；旧镜像接口缺失时，管理端能明确显示版本不匹配，而不是让按钮静默失败。
