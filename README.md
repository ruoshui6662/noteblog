...
完整步骤见 [Docker、GitHub Actions 与飞牛测试指南](docs/DOCKER_GITHUB_FNOS.md)、[Docker/飞牛开发经验记录](docs/DEVELOPMENT_EXPERIENCE.md)、[管理端工作区设计与开发规划](docs/ADMIN_WORKSPACE_PLAN.md)、[M1 阅读端进度](docs/progress/PHASE_01_READER.md) 和 [M2 身份进度](docs/progress/PHASE_02_ADMIN.md)。将项目推送到 GitHub 的 `main` 或 `master` 后，工作流通过检查即发布 `ghcr.io/用户名/仓库名:edge`；飞牛使用 `deploy/compose.yaml` 拉取测试。
